cwL <- read.table("cf_all.mat", sep="\t", header=TRUE, row.names=1)

cwLf <- cwL[,colSums(cwL[,2:ncol(cwL)]) > 25000]
...
hcL <- hclust(d2)
all_order <- hcL$order
write.table(hcL$order, file=("cf_all_order_full.txt"), row.names=FALSE, col.names=FALSE)
all_order <- read.table("cf_all_order_full.txt", header=FALSE)

d_sparse <- dist(cwLC_sparse)
hcL_sparse <- hclust(d_sparse)
sparse_order <- hcL_sparse$order
write.table(hcL_sparse$order, file=("cf_all_order_sparse.txt"), row.names=FALSE, col.names=FALSE)
sparse_order <- read.table("cf_all_order_sparse.txt", header=FALSE)

d_single <- dist(cwLC_single)
hcL_single <- hclust(d_single)
single_order <- hcL_single$order
write.table(hcL_single$order, file=("cf_all_order_single.txt"), row.names=FALSE, col.names=FALSE)
single_order <- read.table("cf_all_order_single.txt", header=FALSE)
...
sparsespan <- GetSpans(cwl_sparseList, lens_sparse, accu_sp, 0, 20)
singlespan <- GetSpans(cwl_singleList, lens_single, accu_si, 0, 20)

namvec_sparse <- GetScaffs(cwl_sparseList, lens_sparse, accu_sp, 0, 20)
namvec_single <- GetScaffs(cwl_singleList, lens_single, accu_si, 0, 20)
namvec <- GetScaffs(cwlList, lens, accu, 1, 20)

write.table(fullspan, file="cf_span_full.txt", sep="\t", quote=FALSE, row.names=FALSE)
write.table(sparsespan, file="cf_span_sparse.txt", sep="\t", quote=FALSE, row.names=FALSE)
write.table(singlespan, file="cf_span_single.txt", sep="\t", quote=FALSE, row.names=FALSE)

write.table(namvec, file="cf_namvec_full.txt", sep="\t", quote=FALSE, row.names=FALSE)
write.table(namvec_sparse, file="cf_namvec_sparse.txt", sep="\t", quote=FALSE, row.names=FALSE)
write.table(namvec_single, file="cf_namvec_single.txt", sep="\t", quote=FALSE, row.names=FALSE)
