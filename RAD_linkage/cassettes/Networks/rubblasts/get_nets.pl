...
{
	my $spanA = $_;
	my @spanAll2 = keys %spans;
	foreach(@spanAll2)
	{
		my $spanB = $_;
		if(exists $spanSpans{$spanA}{$spanB})
		{
			my $in = $spanSpans{$spanA}{$spanA};
			my $out = $spanExternal{$spanA};
			my $all = $in + $out;
			
			my $inB = $spanSpans{$spanB}{$spanB};
			my $outB = $spanExternal{$spanB};
			my $allB = $inB + $outB;
			
			my $expected = (($allB / $pepCount) * (1 - $avgRatio)) * $all;
			
			if($spanSpans{$spanA}{$spanB} > 5 && $spanA ne $spanB)
			{
				my $factor = $spanSpans{$spanA}{$spanB} / $expected;
				my $outRate = $spanSpans{$spanA}{$spanB} / $out;
				
				if($factor > 20)
				{
					print $spanA . "\t" . $spanB . "\t" . $spanA . "\t" . $spanB . "\t" . $all .  "\t" . $out . "\t" . $spanSpans{$spanA}{$spanB} . "\t" . ($outRate * 100) . "\t" . $expected . "\t" . $factor . "\n";
				}
			}
		}
	}
